...
// === adr ===
exports.emaAdr=function(ema){
var stm=db.prepare('select * from adr where email=? ');
try{var row=stm.get(ema);}catch(err){console.log(err)}
return row;
};
...
// === tmp ===
exports.emaTmp=function(arg2){
var stm=db.prepare('select * from tmp where ema=?');
try{var row=stm.get(arg2);
}catch(err){console.log(err);}
return row;
};
...
// === tmp run ===
exports.upSst=function(sst,ema){
var stm=db.prepare('update tmp set sst=? where ema=? ');
try{stm.run(sst,ema);
}catch(err){console.log(err);}
};

exports.inTmp=function(name,email,sst,fst){
var stm=db.prepare('insert into tmp values(?,?,?,?) ');
try{stm.run(name,email,sst,fst);
}catch(err){console.log(err);}
};
